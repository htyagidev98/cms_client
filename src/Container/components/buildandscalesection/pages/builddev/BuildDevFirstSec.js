...
import FirstSecBuildDevModal from './FirstSecBuildDevModal';
...
const BuildDevFirstSec = () => {
    ...
    const fetchHeroData=async()=>{
       try {
          const res= await axios.get(`${BASE_URL}/dev/portal/get`)
    ...
    return (
        <div className='text-white'>
           ...
           <div dangerouslySetInnerHTML={{ __html: cardData.title }}></div>
            ...
            {
            CardModal && <FirstSecBuildDevModal show={CardModal} setUpdateUi={setUpdateUi} hide={closeModal} objectData={objectData} />
           }
        </div>
    )
}
